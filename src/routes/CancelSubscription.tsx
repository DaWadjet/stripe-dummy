...
import CartItem, {
  ItemData,
  ServerSubscriptionsResponseType,
...
function CancelSubscription() {
      ...
      .then((r: ServerSubscriptionsResponseType[]) => {
        ...
        r.forEach((subscriptionItem) => {
          let subscriptionDetails =
            Subscriptions.find(
              (elem) => elem.id === subscriptionItem.appProductId
            ) || undefined;
  ...
  const removeSubscription = (id: string | undefined) => {
    const newSubscriptionsList = subscriptions.filter(
      (elem) => elem.stripeSubscriptionData?.subscriptionId !== id
    );
          ...
          {subscriptions.length === 0 ? (
            <>
              <Input
                ...
                value={email}
              />
              <Button onClick={listSubscriptions} colorScheme={"green"}>
                List Subscriptions
              </Button>
            </>
          ) : (
          ...
          {subscriptions.map((elem) => {
            return (
              <CartItem
                ...
                onCancelled={() =>
                  removeSubscription(
                    elem.stripeSubscriptionData?.subscriptionId
                  )
                }
              />
            );
